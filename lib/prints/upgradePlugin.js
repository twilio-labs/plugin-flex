...
const upgradeNotification = (logger) => async (skip) => {
    flex_plugins_utils_logger_1.boxen.warning('You are about to upgrade your plugin to use the latest version of Flex Plugin CLI.');
    if (!skip) {
        const answer = await flex_plugins_utils_logger_1.confirm('Please backup your plugin either locally or on GitHub. Do you want to continue?');
        if (!answer) {
            general_1.exit(0);
        }
    }
 ...
 * Script started
 */
const scriptStarted = (logger) => (version) => {
 ...
 * Script succeeded
 */
const scriptSucceeded = (logger) => (needsInstall) => {
    logger.newline();
 ...
 */
const updatePluginUrl = (logger) => (newline) => {
    if (newline) {
        logger.newline();
    }
    logger.info(flex_plugins_utils_logger_1.singleLineString('> !!Could not update {{public/appConfig.js}} because your pluginService url has been modified.', "Please update the pluginService url to '/plugins'.", 'You may take a look at {{public/appConfig.example.js}} for guidance.!!'));
};
/**
 * Cannot remove craco because it has been modified
 */
const cannotRemoveCraco = (logger) => (newline) => {
    if (newline) {
        logger.newline();
    }
    logger.info(flex_plugins_utils_logger_1.singleLineString('> !!Cannot remove {{craco.config.js}} because it has been modified from its default value.', `Please review @@${cracoUpgradeGuideLink}@@ for more information.!!`));
};
/**
 * Required package not found
 */
const packageNotFound = (logger) => (pkg) => {
    logger.newline(2);
 ...
 */
const notAvailable = (logger) => (version) => {
 ...
 */
const warnNotRemoved = (logger) => (note) => {
    logger.newline();
 ...
 */
// eslint-disable-next-line @typescript-eslint/no-unused-vars
const removeLegacyNotification = (logger) => async (pluginName, skip) => {
    const name = flex_plugins_utils_logger_1.coloredStrings.name(pluginName);
    flex_plugins_utils_logger_1.boxen.warning(`You are about to delete your legacy plugin ${name} bundle hosted on Twilio Assets.`);
    if (!skip) {
        const answer = await flex_plugins_utils_logger_1.confirm('Please confirm that you have already migrated this plugin to use the Plugins API. Do you want to continue?');
        if (!answer) {
            general_1.exit(0);
        }
    }
 ...
 * @param logger
 */
const noLegacyPluginFound = (logger) => (pluginName) => {
    const name = flex_plugins_utils_logger_1.coloredStrings.name(pluginName);
 ...
 * @param logger
 */
const removeLegacyPluginSucceeded = (logger) => (pluginName) => {
    const name = flex_plugins_utils_logger_1.coloredStrings.name(pluginName);
    logger.newline();
 ...
 */
const warningPluginNotInAPI = (logger) => (pluginName) => {
    const name = flex_plugins_utils_logger_1.coloredStrings.name(pluginName);
