...
class FlexPluginsUpgradePlugin extends flex_plugin_1.default {
    ...
    async cleanupNodeModules() {
        await flex_plugins_utils_logger_1.progress('Cleaning up node_modules and lock files', async () => {
    ...
    async npmInstall() {
        ...
        await flex_plugins_utils_logger_1.progress(`Installing dependencies using ${cmd}`, async () => {
            const args = ['install'];
            if (this._flags.yarn) {
                args.push('--silent');
            }
            ...
            const { exitCode, stderr } = await flex_plugins_utils_spawn_1.default(cmd, args);
            if (exitCode || stderr) {
...
FlexPluginsUpgradePlugin.flags = Object.assign(Object.assign({}, baseFlags), { 'remove-legacy-plugin': parser_1.flags.boolean({
        ...
        description: commandDocs_json_1.upgradePlugin.flags.yarn,
